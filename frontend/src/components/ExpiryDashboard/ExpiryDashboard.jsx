...
const timeLeft = (days) => {
  if (days < 0)
    return `Expired ${Math.abs(Math.round(days))} day${Math.abs(Math.round(days)) !== 1 ? 's' : ''} ago`;
...
const getRecommendedAction = (status, qty) => {
  if (status === 'expired')
    return { label: 'Discard immediately', color: '#dc2626' };
  if (status === 'today' && qty > 20)
    return { label: 'Move to discount shelf', color: '#ea580c' };
  if (status === 'today') return { label: 'Prioritize sale', color: '#ea580c' };
  if (status === 'soon' && qty > 30)
    return { label: 'Apply discount', color: '#ca8a04' };
...
const getWasteRisk = (status, qty) => {
  ...
  if ((status === 'today' || status === 'soon') && qty > 30)
    return { label: 'High', color: '#ea580c', bg: '#ffedd5' };
  if ((status === 'today' || status === 'soon') && qty > 10)
                    ...
                    <tr key={`group-${name}`} className="group-header-row">
                      <td colSpan={9}>
                        ...
                        <span className="group-meta">
                          ...
                          Nearest expiry:{' '}
                          {new Date(batches[0].expiryDate).toLocaleDateString()}
                        </span>
                      </td>
                    </tr>
