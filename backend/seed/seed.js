...
const seed = async () => {
  try {
      ...
      const batches = Array.from({ length: numBatches }, () => {
        ...
        return {
          ...
          expiryDate: randomDate(
            faker.number.int({ min: range.min, max: range.max }),
          ),
    ...
    console.log(
      "✅ Seeding complete! Total records:",
      users.length + products.length + wasteReports.length,
    );
  } catch (err) {
